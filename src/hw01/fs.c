...
{
  char* pBuff = calloc(INODE_NUM_PER_BLOCK, sizeof(Inode));
  /**
    inodeno - blockno 
    0 ~ 15 - 3
    16 ~ 31 - 5 
  ...
  DevReadBlock(blockno, pBuff);
  // copy pInode to pBuffer
  ((Inode*)pBuff)[inodeIndex]=*pInode;
  DevWriteBlock(blockno, pBuff);
  free(pBuff);
}
...
{
  int* pBuff = calloc(BLOCK_SIZE/entrySize, entrySize);
...
{
  DirEntry* pBuff = calloc(BLOCK_SIZE/sizeof(DirEntry), sizeof(DirEntry));
  DevReadBlock(blkno, (char*)pBuff);
  pBuff[index]=*pEntry;
  DevWriteBlock(blkno, (char*)pBuff);
  free(pBuff);
